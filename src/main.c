...
int main(int argc, char *argv[])
{
	...
	if((status = uci_cmd("uci get devcfg.internet.mode", "wan", net_mode, 20)) < 0) {
		printf("uci get net_mode failed!\n");
		exit(-1);
	}
	if((status = uci_cmd("uci get devcfg.internet.apn", "3gnet", net_apn, 20)) < 0) {
		printf("uci get net_apn failed!\n");
		exit(-1);
	}
	...
	char buf[POPENBUFLEN];
	char syscmd[256];
	strcpy(syscmd,"ping -q -c 3 www.baidu.com -w 5 | grep 'packet loss' | awk -F\"[ %]\" '{print $7}'");  //pppoe   username
	printf("cmd:%s\n",syscmd);
	...
	long *shared;//指向shm
	//创建共享内存
	shmid = shmget((key_t)7688, sizeof(long), 0666|IPC_CREAT);
	if(shmid == -1)
	{
	...
	printf("\n%s Memory attached at [%d]%X\n", argv[0],shmid, (int*)shm);
	//设置共享内存
	shared = (long*)shm;
	...
	while(1){
		...
		if (loss_rate>50){	//失败率大于50%
			...
			//ifup wan4
			if (reset_count>=3){		//失败率50%三次，则复位4G模块和系统网络
				//system("ifup wan4");
				while(1){
					...
					//start quectel-CM
					if (strcmp(net_mode,"4g") == 0){
						system(quectel_CM_cmd);		//启动quectel_CM进程，4G拨号上网
					}
		...
		printf("clock: *shared = %ld, local_clock = %ld (diff = %d)\n", *shared, get_sys_clock_ms(), abs((*shared)-(get_sys_clock_ms())));

		if (net_state == 1){	//网络是连接上的情况
			if (check_mqttgw_stat){
				if ((get_sys_clock_ms() - *shared) > 100*1000){
/*					printf("restart mqttgw\n");
					system("/root/mqttgw -l");
					sleep(2);
					system("/root/mqttgw -b");*/

					printf("restart mt7688_lora_gateway\n");
					//system("/root/lora/lora_pkt_fwd -l");
					//killall lora_pkt_fwd
					chdir("/root/lora/");
					system("/root/lora/mt7688_lora_gateway -l");
					sleep(2);
					status = system("/root/lora/mt7688_lora_gateway -b");
					if(status < 0) {
						printf("start mt7688_lora_gateway failed!\n");
						exit(-1);
					}
					chdir("/");
					printf("start mt7688_lora_gateway successful!\n");
				}
			}else{
				check_mqttgw_stat = 1;
			}
